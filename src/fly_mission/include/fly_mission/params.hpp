...
//    LAND         降落，任务结束
// ---------------------------------------------------------------------------
// 第二段任务总开关：false = 第一段降落完直接结束(改动前行为)。
inline constexpr bool   MISSION2_ENABLE   = true;
// 第二段任务的"追上确认时长"(s)：与第一段 CATCH_HOLD_SEC 分开，互不影响。
//   累计在 CATCH_DIST 内达此时长 → 开始边追边降。
inline constexpr double CATCH_HOLD_SEC_2  = 12.0;
...
//     ★接触计时被清零★，而 PLAT_TOUCH_HOLD_S=1.0s 要求【连续 50 拍】都在闸门内，
...
// ★★★ 下降速率的估计窗口 (s) ★★★
//   ★为什么必须有★(2026-08 实测 bug：落到平台了却不锁桨、进不了 5 秒倒计时)：
//   原来用【单拍差分】算速度：vz = (上一拍高度 - 当前高度) / 0.02s。
//   问题是 SLAM 高度噪声哪怕只有 ±1cm，算出的假速度就是 0.01/0.02 = ★0.5 m/s★，
//   是阈值 PLAT_TOUCH_VZ(0.05) 的【10 倍】。于是飞机明明已经停在平台上，
//   每拍抖动都让 vz 忽大忽小，只要一拍超阈值就把接触计时清零 →
//   永远凑不满 PLAT_TOUCH_HOLD_S → ★永远不锁桨★。
//   ★滑动窗口解决★：拿本值秒之前的高度和现在比，噪声被时间跨度稀释 ——
//   同样 ±1cm 噪声，0.3s 窗口只产生 0.01/0.3 = 0.033 m/s 假速度，落在阈值内。
//   ★取值★：要 >> 单拍周期(0.02s)才有滤波效果；又不能太长(响应变慢、接触后要多
//   等一会儿才判出来)。0.3s = 15 拍，是滤噪与响应的折中。
inline constexpr double PLAT_VZ_WIN_S      = 0.5;

// ★"已确认真的降起来了"的最小降速 (m/s)★：只有实际降速超过此值，才启用接触检测。
//   ★为什么要它★：刚进边追边降时飞机还在悬停(降速≈0)，若不设这道门，会立刻满足
//   "降不动"→ 在高空锁桨。有了它，必须先看到飞机确实在往下走。
//   ★为什么不用 PLAT_DESCEND_SPD*0.5 推算★：那个是【限速/目标压降速率】，不等于
//   实际能降多快(受载重/PD/飞控限制)。拿它推门槛的话，一旦实际降速达不到，
//   这道门就永远过不去 → ★接触检测永不启用 → 永不锁桨★(2026-08 实测踩过)。
//   ★取值★：明显大于 SLAM 噪声在 PLAT_VZ_WIN_S 窗口下的假速度、又远小于正常降速。
//   0.05 = 每秒至少降 5cm 就算"在降"。
//   ★必须明显大于 PLAT_TOUCH_VZ★：两者相等会没有余量——降速在阈值附近抖动时，
//   会在"启用检测"和"判定接触"之间来回跳。取 PLAT_TOUCH_VZ 的 2~3 倍。
//   ★也必须明显小于实际能达到的降速★(受 PLAT_DESCEND_SPD 限速和载重影响)，
//   否则这道门永远过不去 → 接触检测永不启用 → 永不锁桨。
inline constexpr double PLAT_DESCEND_MIN_VZ = 0.25;

// 判定"实际高度不再下降"的速率阈值 (m/s)：实际下降速率低于此值就算"降不动了"。
//   ★只在高度已低于 PLAT_DISARM_MAX_H_REL 时才参与判定★(见上)。
//   ★与 PLAT_VZ_WIN_S 配合★：窗口越长噪声越小，这个阈值就能取得越小越灵敏。
//   ★取值依据(0.5s 窗口下)★：±10mm 噪声 → 假速度 0.01/0.5 = 0.02 m/s；
//     ±20mm → 0.04 m/s。取 0.10 对两者都有 2.5 倍以上余量。
//   0.10 = 每秒降不到 10cm 就认为被托住了。
//   ★太小★ → 噪声就能超过它 → 接触计时被反复清零 → ★永远判不出接触、不锁桨★
//     (2026-08 实测：0.05 配 0.3s 窗口时，±10mm 噪声就锁不上)。
//   ★太大★ → 正常下降途中被误判成已接触(但有高度闸门 PLAT_DISARM_MAX_H_REL 兜底，
//     不会真的在高空锁桨，只是可能提前上锁一点)。
inline constexpr double PLAT_TOUCH_VZ      = 0.10;
// 接触判定持续时长 (s)：连续这么久都"降不动"才判定真接触(防瞬时噪声误判)。
//   ★别太短★：容易被一次高度跳变骗到 → 提前上锁(有高度闸门兜底，但仍不该发生)。
//
//   ★★★ 从"物理接触"到"真正锁桨"的总延迟 ★★★
//       = PLAT_VZ_WIN_S(滑窗滞后) + 本值 = 0.5 + 1.0 = ★约 1.5 秒★
//   为什么有滑窗那 0.5s：接触瞬间，滑窗里还留着 0.5s 前(仍在下降)的旧样本，
//   算出的 vz 仍然偏大；要等窗口被"停住后"的样本填满，vz 才降到 ~0 开始累计本时长。
