...
inline constexpr double EXPLORE_GOAL_X = 1.55;   // 原探索终点（图中红点）
inline constexpr double EXPLORE_GOAL_Y = 0.60;
inline constexpr bool   EXPLORE_CORRIDOR_ENABLED = true; // false: 红点完成后按旧流程降落
inline constexpr double CORRIDOR_ENTRY_X = 3.24;  // 保持 -90 度朝向横移进入通道的第一个点 x
inline constexpr double CORRIDOR_ENTRY_Y = 0.6;  // 进入通道的第一个点 y
inline constexpr double CORRIDOR_H_X = 3.13;      // 穿过全部门后的 H 点 x
inline constexpr double CORRIDOR_H_Y = -3.86;     // 穿过全部门后的 H 点 y
